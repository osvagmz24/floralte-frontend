...
function buildOrderSummary(order) {
  ...
  for (const item of items) {
    ...
    const description = text(item?.d || item?.description || "", 220);
    if (description && !flowers) lines.push(`  ${description}`);

    const style = text(item?.style || "", 60);
    if (style) lines.push(`  Estilo: ${style}`);
  }

  const customerPhone = text(order?.customer?.phone || "", 40);
  if (customerPhone) {
    lines.push("");
    lines.push(`WhatsApp cliente: ${customerPhone}`);
  }
...
function collectPreviewImages(order) {
  const items = Array.isArray(order?.items) ? order.items : [];
  return items
    .filter(i => publicHttpsUrl(i?.preview))
    .map(i => {
      const flowers = bouquetLine(i);
      const description = text(i?.d || i?.description || "", 170);
      const detail = flowers ? `Flores: ${flowers}` : (description || "Diseño Floralte");
      return {
...
async function sendOrderNotification({ to, phoneNumberId, token, payment, order }) {
  ...
  const customerName = text(order?.customer?.name || "Sin nombre", 100);
  const customerPhone = text(order?.customer?.phone || "", 40);
  const customer = text(`${customerName}${customerPhone ? ` · WhatsApp ${customerPhone}` : ""}`, 160);
        ...
        components: [
          ...
          },
          {
            ...
            parameters: [
              ...
              { type: "text", text: text(`${image.price} · ${image.detail}`, 700) }
            ]
          }
        ]
      }
