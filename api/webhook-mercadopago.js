...
async function graphPost(phoneNumberId, token, payload) {
  ...
  if (!response.ok) {
    ...
    const message = data?.error?.message || data?.message || `WhatsApp respondió ${response.status}`;
...
module.exports = async function handler(req, res) {
  ...
  // El simulador del panel de Mercado Pago envía live_mode:false y el Data ID
  // que tú escribes en la prueba. Ese ID puede no corresponder a un pago real,
  // así que NO intentamos consultarlo en /v1/payments/{id}. Solo confirmamos
  // la recepción con 200, que es lo que Mercado Pago espera para validar la URL.
  // Las compras productivas reales llegan con live_mode:true y siguen al flujo
  // normal: consultar pago -> validar approved -> enviar WhatsApp.
  if (body?.live_mode === false && String(body?.type || type || '').toLowerCase() === 'payment') {
    return res.status(200).json({
      ok: true,
      simulated: true,
      received: true,
  ...
  // MODO SIMULADOR SEGURO:
  // Configura únicamente la URL de PRUEBAS de Mercado Pago como:
  // https://floralte.com/api/webhook-mercadopago?simulator=1
  // La URL de PRODUCCIÓN debe quedarse SIN este parámetro.
  //
  // El simulador de Mercado Pago usa IDs ficticios, por lo que no existe un
  // pago real para consultar en /v1/payments/{id}. En este modo validamos la
  // firma y enviamos una plantilla de prueba a WhatsApp para comprobar todo
  // el flujo Mercado Pago -> Vercel -> WhatsApp sin tocar compras reales.
  const simulatorMode = getQueryValue(req, "simulator") === "1";
  if (simulatorMode) {
    try {
      ...
      console.error("Prueba simulador Mercado Pago -> WhatsApp falló:", error);
      return res.status(500).json({
        ok: false,
        simulated: true,
