...
async function graphPost(phoneNumberId, token, payload) {
  ...
  const cleanTo = String(payload?.to || "").replace(/\D/g, "");
  const safePayload = { ...payload, to: cleanTo };
  const url = `https://graph.facebook.com/${GRAPH_VERSION}/${encodeURIComponent(cleanPhoneNumberId)}/messages`;

  const params = safePayload?.template?.components?.find(c => c?.type === "body")?.parameters || [];
  console.log("WhatsApp request debug:", JSON.stringify({
    graphVersion: GRAPH_VERSION,
    phoneNumberId: cleanPhoneNumberId,
    toLast4: cleanTo.slice(-4),
    type: safePayload?.type,
    templateName: safePayload?.template?.name,
    language: safePayload?.template?.language?.code,
    bodyParameterCount: params.length,
    bodyParameterLengths: params.map(p => String(p?.text ?? "").length)
  }));
  ...
  if (!response.ok) {
    console.error("WhatsApp API error FULL:", raw || JSON.stringify(data));
    ...
    throw new Error(`Meta WhatsApp ${code}${subcode}: ${details}`);
  }
...
function whatsappTemplateText(value, max = 900) {
  return String(value ?? "")
    .replace(/[\r\n\t]+/g, " ")
    .replace(/\s{2,}/g, " ")
    .trim()
    ...
    return graphPost(phoneNumberId, token, {
      ...
      template: {
        ...
        components: [{
          type: "body",
          parameters: [
            { type: "text", text: whatsappTemplateText(reference, 90) },
            { type: "text", text: whatsappTemplateText(total, 90) },
            { type: "text", text: whatsappTemplateText(customer, 100) },
            { type: "text", text: whatsappTemplateText(summary, 900) }
          ]
        }]
      }
    });
