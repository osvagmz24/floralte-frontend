...
function mercadoPagoPhone(value) {
  ...
  const twoDigitLadas = new Set(["33", "55", "81"]);
  const areaCode = twoDigitLadas.has(digits.slice(0, 2)) ? digits.slice(0, 2) : digits.slice(0, 3);
  const localNumber = digits.slice(areaCode.length);
  return { area_code: areaCode, number: Number(localNumber) };
}
...
function makeSnapshot(body, cleanItems, externalReference) {
  ...
  const items = cleanItems.slice(0, 30).map((item, index) => {
    ...
    return {
      ...
      d: text(original.description || body.items?.[index]?.description || item.description || "", 240),
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const customerPhone = normalizeMexicanPhone(body?.customer?.phone || body?.order?.customer?.phone);
    if (!customerPhone) {
      return res.status(400).json({ ok: false, error: "El WhatsApp del cliente es obligatorio y debe tener 10 dígitos." });
    }
    ...
    snapshot.customer.phone = customerPhone;
    ...
    const payerPhone = mercadoPagoPhone(customerPhone);
    preference.payer = {};
    if (customerName) preference.payer.name = customerName;
    if (payerPhone) preference.payer.phone = payerPhone;
